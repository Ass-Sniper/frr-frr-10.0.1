...
DEFINE_HOOK(bgp_snmp_update_stats,
	    ...
	    (rn, pi, added));
/*
struct hook _hook_bgp_snmp_update_stats = {
    .name = "bgp_snmp_update_stats",
    .entries = ((void * ) 0),
    .reverse = 0,
};
static int hook_call_bgp_snmp_update_stats(struct bgp_dest * rn, struct bgp_path_info * pi, _Bool added) {
    int hooksum = 0;
    struct hookent * he = _hook_bgp_snmp_update_stats.entries;
    void * hookarg;
    union {
        void * voidptr;
        int( * fptr)(struct bgp_dest * rn, struct bgp_path_info * pi, _Bool added);
        int( * farg)(void * hookarg, struct bgp_dest * rn, struct bgp_path_info * pi, _Bool added);
    } hookp;
    for (; he; he = he -> next) {
        hookarg = he -> hookarg;
        hookp.voidptr = he -> hookfn;
        if (!he -> has_arg) hooksum += hookp.fptr(rn, pi, added);
        else hooksum += hookp.farg(hookarg, rn, pi, added);
    }
    return hooksum;
}
...
/*
struct hook _hook_bgp_process = {
    .name = "bgp_process",
    .entries = ((void * ) 0),
    .reverse = 0,
};
static int hook_call_bgp_process(struct bgp * bgp, afi_t afi, safi_t safi, struct bgp_dest * bn, struct peer * peer, _Bool withdraw) {
    int hooksum = 0;
    struct hookent * he = _hook_bgp_process.entries;
    void * hookarg;
    union {
        void * voidptr;
        int( * fptr)(struct bgp * bgp, afi_t afi, safi_t safi, struct bgp_dest * bn, struct peer * peer, _Bool withdraw);
        int( * farg)(void * hookarg, struct bgp * bgp, afi_t afi, safi_t safi, struct bgp_dest * bn, struct peer * peer, _Bool withdraw);
    } hookp;
    for (; he; he = he -> next) {
        hookarg = he -> hookarg;
        hookp.voidptr = he -> hookfn;
        if (!he -> has_arg) hooksum += hookp.fptr(bgp, afi, safi, bn, peer, withdraw);
        else hooksum += hookp.farg(hookarg, bgp, afi, safi, bn, peer, withdraw);
    }
    return hooksum;
}
_Static_assert(1, "please add a semicolon after this macro")
